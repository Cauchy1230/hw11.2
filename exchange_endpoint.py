...
def trade():
    if request.method == "POST":
        ...
        if result is True:
            this_order = Order(sender_pk=sender_pk, receiver_pk=receiver_pk, buy_currency=buy_currency,
...
def order_book():
    ...
    for this in g.session.query(Order).all():
        result['data'].append(
            {'sender_pk': this.sender_pk, 'receiver_pk': this.receiver_pk, 'buy_currency': this.buy_currency,
             'sell_currency': this.sell_currency, 'buy_amount': this.buy_amount, 'sell_amount': this.sell_amount,
